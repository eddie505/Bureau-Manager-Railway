...
function NuevoReciboExcel() {
  ...
  useEffect(() => {
    if (selectedCondominio) {
      axios.post(`http://localhost:4000/api/getEdificiosbyCondominio`, { id_condominio: selectedCondominio })
        .then(response => {
  ...
  const handleSubmit = async (event) => {
    ...
    try{
      ...
      const checkInquilinosPromises = deptosBD.map(depto => 
        axios.post('http://localhost:4000/api/getInquilinosbyDepartamento', { id_departamento: depto.id_departamento })
      );

      const resultsInquilino = await Promise.all(checkInquilinosPromises);
      const deptosSinInquilinos = resultsInquilino.map((result, index) => {
        if (result.data.length === 0) return deptosBD[index].numero_departamento;
        return null;
      }).filter(Boolean);
  ...
  function convertirFechaExcel(serial) {
    ...
    const fechaReal = new Date(fechaBase.getTime() + diasAjustados * milisegundosPorDia);
    ...
    <div className='div-contenedor div-espaciado'>
      ...
      <form className='fromInquilino' onSubmit={handleSubmit}>
          ...
          <div className='select-item'>
            ...
            <select onChange={handleCondominioChange} value={selectedCondominio}>
              ...
              {condominios.map((condominio) => (
                <option key={condominio.id_condominio} value={condominio.id_condominio}>
        ...
        <div className='botones-container'>
            <button className='mi-boton2' type='submit'>Generar recibos</button>
        </div>
      </form>
