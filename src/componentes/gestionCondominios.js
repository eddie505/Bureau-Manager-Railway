...
function CondominiosComponent() {
    ...
    const selectCondominio = async (condominio) => {
        if (selectedCondominio && selectedCondominio.id_condominio === condominio.id_condominio) {
            setSelectedCondominio(null);
            setSelectedEdificio(null);
        } else {
            try {
                const response = await axios.post(`${config.REACT_APP_API_BASE_URL}/api/getEdificiosbyCondominio`, { id_condominio: condominio.id_condominio });
    ...
    const selectEdificio = async (edificio) => {
        if (selectedEdificio && selectedEdificio.id_edificio === edificio.id_edificio) {
            setSelectedEdificio(null);
        } else {
            try {
                const response = await axios.post(`${config.REACT_APP_API_BASE_URL}/api/getDepartamentosbyEdificios`, { id_edificio: edificio.id_edificio });
    ...
    const fetchInquilinosByCondominio = async (id_condominio) => {
        try {
          const response = await axios.get(`${config.REACT_APP_API_BASE_URL}/api/getInquilinosByCondominio?id_condominio=${id_condominio}`);
        ...
        <div style={{ marginTop: "100px" }}>
          ...
          {condominios.map((condominio, index) => (
            <div key={condominio.id_condominio}>
              <button onClick={() => selectCondominio(condominio)} className="condominio-btn">
                {condominio.nombre_condominio}
                <span style={{ float: 'right' }}>
                  {selectedCondominio && selectedCondominio.id_condominio === condominio.id_condominio ? <IoIosArrowUp /> : <IoIosArrowDown />}
                </span>
              </button>
              {selectedCondominio && selectedCondominio.id_condominio === condominio.id_condominio && (
                <div style={{padding: "10px"}}>
                  <p style={{textAlign: "left"}}>Dirección: {selectedCondominio.direccion_condominio}</p>
                  <p style={{textAlign: "left"}}>Número de edificios registrados del condominio: {selectedCondominio.edificios.length}</p>
                  <p style={{textAlign: "left"}}>Administrador a cargo: {selectedCondominio.admin_condominio}</p>
                  <br/>
                  <h4>Edificios: </h4>
                  <br/>
                  <ul>
                    {selectedCondominio.edificios.map(edificio => (
                      <li key={edificio.id_edificio}>
                        <button onClick={() => selectEdificio(edificio)} className="edificio-btn">
                          {edificio.nombre_edificio}
                          <span style={{ float: 'right' }}>
                            {selectedEdificio && selectedEdificio.id_edificio === edificio.id_edificio ? <IoIosArrowUp /> : <IoIosArrowDown />}
                          </span>
                        </button>
                        {selectedEdificio && selectedEdificio.id_edificio === edificio.id_edificio && (
                          <div>
                            ...
                            <div className="departamentos-lista">
                                <h4>Departamentos: </h4>
                                <br/>
                              {selectedEdificio.departamentos.map((departamento, index, array) => (
                                <span key={departamento.id_departamento} className="departamento-item">
                                  {departamento.numero_departamento}{index < array.length - 1 ? ', ' : ''}
                                </span>
                              ))}
                              <br/>
                            </div>
